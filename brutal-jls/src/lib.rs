...
impl BrutalControl {
    ...
    fn set_parameter(&self, param: CongestionParameter) {
        match param {
            CongestionParameter::PeerBandwidthHint(v) => {
                *self.peer_bandwidth_hint.lock().unwrap() = Some(v);
            }
            CongestionParameter::CwndGain(v) => {
                *self.cwnd_gain.lock().unwrap() = Some(v);
            }
            CongestionParameter::AckCompensation(v) => {
                *self.ack_compensation.lock().unwrap() = Some(v);
            }
        }
    }

    fn take_peer_bandwidth_hint(&self) -> Option<u64> {
        self.peer_bandwidth_hint.lock().unwrap().take()
    }

    fn take_cwnd_gain(&self) -> Option<f64> {
        self.cwnd_gain.lock().unwrap().take()
    }

    fn take_ack_compensation(&self) -> Option<bool> {
        self.ack_compensation.lock().unwrap().take()
    }
}
...
impl Brutal {
    fn apply_pending_parameters(&mut self) -> Result<(), ConfigError> {
        if let Some(v) = self.control.take_peer_bandwidth_hint() {
            self.set_parameter(CongestionParameter::PeerBandwidthHint(v))?;
        }

        if let Some(v) = self.control.take_cwnd_gain() {
            self.set_parameter(CongestionParameter::CwndGain(v))?;
        }

        if let Some(v) = self.control.take_ack_compensation() {
            self.set_parameter(CongestionParameter::AckCompensation(v))?;
        }
...
impl Controller for Brutal {
    ...
    fn set_parameter(&mut self, param: CongestionParameter) -> Result<(), ConfigError> {
        ...
        trace!(
            "[brutal] set_parameter applied: remote={}, cwnd={}, target_bps={}, ack_comp={}, cwnd_gain={}",
            self.remote,
            self.core.cwnd,
